...
function resolveItemServerPrice(itemType, metadata, isBonus) {
  if (isBonus) return 0;
  if (itemType === 'sky_map') return 2000;
  if (itemType === 'bonus_photo') return 0;
  if (itemType === 'bonus_ambiance') return 0;
  if (itemType === 'ambiance') return 199;
  if (itemType === 'photo') return resolvePhotoPrice(metadata || {});
  return null;
}
...
function annotateCartPricing(items) {
  ...
  const photos = items.filter(i => i.type === 'photo' || i.type === 'bonus_photo');
  ...
  const paidAmbianceIndexes = [];
  for (let i = 0; i < annotatedItems.length; i++) {
    const item = annotatedItems[i];
    if (item.type === 'ambiance' && !item.isBonus) paidAmbianceIndexes.push(i);
  }
  if (paidAmbianceIndexes.length > 0) {
    const prices = calculateAmbiancePriceDistribution(paidAmbianceIndexes.length);
    for (let i = 0; i < paidAmbianceIndexes.length; i++) {
      const idx = paidAmbianceIndexes[i];
      annotatedItems[idx] = {
        ...annotatedItems[idx],
        displayPrice: prices[i] || 0,
        ambianceBundleApplied: true,
        ambianceBundleCount: paidAmbianceIndexes.length,
        ambianceBundlePrice: prices[i] || 0
      };
    }
  }

  const hasSkyMap = annotatedItems.some(i => i.type === 'sky_map');
  const hasPhoto = annotatedItems.some(i => i.type === 'photo' || i.type === 'bonus_photo');
  const bonusAmbianceCount = (hasSkyMap ? 1 : 0) + (hasPhoto ? 1 : 0);
  if (bonusAmbianceCount > 0) {
    const paidAmbianceKeys = annotatedItems
      .filter(i => i.type === 'ambiance' && !i.isBonus)
      .map(i => extractAmbianceKeyFromMetadata(i.metadata || {}))
      .filter(Boolean);
    const seed = JSON.stringify({
      ids: annotatedItems.map(i => i.id || i.title || i.type),
      skyMapsInCart,
      freePhotoCount,
      bonusAmbianceCount
    });
    ...
    selectedBonus.forEach((bonus, idx) => {
      ...
      annotatedItems.push({
        id: `auto_bonus_ambiance_${bonus.ambianceId}_${idx}`,
        type: 'bonus_ambiance',
        title: source === 'card'
...
async function addItem(req, res) {
  ...
  const serverPrice = resolveItemServerPrice(type, metadata || {}, false);
  if (serverPrice !== null) {
    finalPrice = serverPrice;
  }
  if (type === 'photo' && serverPrice === null) {
